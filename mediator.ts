...
async function initProtocolByMediator(){
  while ( true ) {
    ...
    const missingRoles = allRoles.filter(
      (role) => connectedRoles.missing(role)
    );

    missingRoles.forEach(
      (role) => console.log(`${role} has not started`)
    );
    ...
    const relevantRoles = allRoles.filter(
      (role) => role !== roles.mediator
    );

    for ( let i=0; i<relevantRoles.length; i++ ) {
      for ( let j=0; j<relevantRoles.length; j++ ) {
        if (i!=j) {
          const rm = new ROLEMESSAGE(relevantRoles[j]);
